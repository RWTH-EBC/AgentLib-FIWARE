...
class ContextBrokerPingPong(BaseModule):
    ...
    def process(self):
        while True:
            self.logger.info("Sending value 'ping' message: %s", self.id)
            self.set(name=self.config.ping.name, value=f'ping-{self.env.time}')
